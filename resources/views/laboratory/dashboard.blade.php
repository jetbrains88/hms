                        ...
                        <div class="border border-gray-200 rounded-xl p-4 hover:bg-blue-50 transition-colors duration-200">
                            <div class="flex justify-between items-center">
                                ...
                                <div class="flex space-x-2">
                                    ...
                                    <a :href="'/laboratory/reports/' + report.id"
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('labDashboard', () => ({
                    ...
                    async fetchStats() {
                        try {
                            const response = await fetch('/api/lab-reports/statistics');
                            if (!response.ok) {
                    ...
                    async refreshQueue() {
                        try {
                            const response = await fetch('/api/lab-reports/pending');
                            if (!response.ok) {
                                throw new Error(`HTTP error! status: ${response.status}`);
                            }
                            const data = await response.json();
                    ...
                    async startProcessing(reportId) {
                        try {
                            const response = await fetch(`/laboratory/reports/${reportId}/status`, {
                                method: 'PUT',
                                headers: {
                    ...
                    async openQuickReportModal() {
                        try {
                            const response = await fetch('/laboratory/reports/create');
                            if (!response.ok) {
                                throw new Error(`HTTP error! status: ${response.status}`);
                            }
                    ...
                    printQueue() {
                        window.open('/laboratory/reports/queue/print', '_blank');
                    },
