...
<head>
    ...
    <title>Lab Report - {{ $labReport->lab_number }}</title>
    <style>
        @php
    ...
    <tr>
        ...
        <td class="patient-info-cell">
            ...
            <div class="patient-info-row">
                ...
                <span>{{ $labReport->patient->emrn }}</span>
            </div>
            <div class="patient-info-row">
                <span class="label">Collection Date:</span>
                <span>{{ $labReport->collection_date ? $labReport->collection_date->format('d-M-Y') : 'N/A' }}</span>
            </div>
            <div class="patient-info-row">
                <span class="label">Reporting Date:</span>
                <span>{{ $labReport->reporting_date ? $labReport->reporting_date->format('d-M-Y') : date('d-M-Y') }}</span>
            </div>
           <div class="patient-info-row">
                <span class="label">Lab No:</span>
                <span>{{ $labReport->lab_number }}</span>
            </div>
        </td>
    </tr>
</table>
...
@if($labReport->hasResults())
    ...
    <table class="results-table">
        ...
        @foreach($labReport->formatted_results as $result)
...
<div style="font-size: 9pt; margin-top: 15px; border-top: 1px solid #ccc; padding-top: 5px;">
    Result generated by {{ $labReport->device_name ?? 'Laboratory' }}
</div>
