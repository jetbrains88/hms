...
@section('content')
            ...
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <div class="flex items-center mt-2 space-x-4">
                        <span class="text-gray-600">Test Code: <strong>{{ $labOrder->test_code }}</strong></span>
                        <span class="px-3 py-1 rounded-full text-sm font-medium {{
    ($labOrder->status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
    ($labOrder->status === 'processing' ? 'bg-blue-100 text-blue-800' :
    ...
    'bg-red-100 text-red-800')))
                ...
                <div class="flex space-x-4">
                    <a href="{{ route('lab.orders.show', $labOrder->id) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-xl p-4">
                            <div class="font-medium text-gray-900">{{ $labOrder->patient->name }}</div>
                            <div class="text-sm text-gray-600">
                                CNIC: {{ $labOrder->patient->cnic }} |
                                EMRN: {{ $labOrder->patient->emrn }} |
                                Age: {{ \Carbon\Carbon::parse($labOrder->patient->dob)->age }} years
                            </div>
                        </div>
                    </div>
                            ...
                            <select x-model="form.lab_test_type_id" id="lab_test_type_id" required
                                ...
                                @foreach($testTypes as $type)
                                    <option value="{{ $type->id }}"
                                            ...
                                            {{ $labOrder->lab_test_type_id == $type->id ? 'selected' : '' }}>
                        ...
                        <div class="relative">
                            <select x-model="form.doctor_id" id="doctor_id" required
                                ...
                                @foreach($doctors as $doctor)
                                    <option
                                        value="{{ $doctor->id }}" {{ $labOrder->doctor_id == $doctor->id ? 'selected' : '' }}>
                                        {{ $doctor->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <select x-model="form.status" id="status"
                                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            <option value="pending" {{ $labOrder->status == 'pending' ? 'selected' : '' }}>Pending</option>
                            <option value="processing" {{ $labOrder->status == 'processing' ? 'selected' : '' }}>Processing</option>
                            <option value="completed" {{ $labOrder->status == 'completed' ? 'selected' : '' }}>Completed</option>
                            <option value="cancelled" {{ $labOrder->status == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea x-model="form.comments" id="comments" rows="4"
                                  class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ $labOrder->comments }}</textarea>
                    </div>
                </div>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('labReportForm', () => ({
                ...
                form: {
                    ...
                    lab_test_type_id: '{{ $labOrder->lab_test_type_id }}',
                ...
                async submitForm() {
                    ...
                    try {
                        const response = await fetch('{{ route("lab.orders.update", $labOrder->id) }}', {
                            method: 'PUT',
                            headers: {
                ...
                async deleteReport() {
                    ...
                    try {
                        const response = await fetch('{{ route("lab.orders.destroy", $labOrder->id) }}', {
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                                'Accept': 'application/json'
