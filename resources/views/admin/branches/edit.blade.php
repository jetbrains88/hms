...
@section('title', 'Edit Branch - NHMP HMS')
@section('page-title', 'Edit Branch')
@section('breadcrumb', 'Edit Branch')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-2xl shadow-soft border border-slate-100 overflow-hidden">
        <div class="p-6 border-b border-slate-100 flex justify-between items-center">
            <h3 class="text-xl font-bold text-slate-800">Edit Branch Information</h3>
            <a href="{{ route('admin.branches.index') }}" class="text-sm text-indigo-600 hover:text-indigo-800 font-bold">
                <i class="fas fa-arrow-left mr-1"></i> Back to List
            </a>
        </div>
        
        <form action="{{ route('admin.branches.update', $branch) }}" method="POST" class="p-8 space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <select name="type" id="type" required
                            ...
                            <option value="CMO" {{ old('type', $branch->type) == 'CMO' ? 'selected' : '' }}>CMO (Central Medical Office)</option>
                            <option value="RMO" {{ old('type', $branch->type) == 'RMO' ? 'selected' : '' }}>RMO (Regional Medical Office)</option>
                        </select>
                    </div>
                    ...
                    <div class="relative group">
                        ...
                        <select name="office_id" id="office_id"
                            ...
                            @foreach($offices as $office)
                                <option value="{{ $office->id }}" {{ old('office_id', $branch->office_id) == $office->id ? 'selected' : '' }}>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center space-x-4 h-[50px] px-4 bg-slate-50 border border-slate-200 rounded-xl">
                        <label class="flex items-center cursor-pointer">
                            <div class="relative">
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $branch->is_active) ? 'checked' : '' }} class="sr-only">
                                <div class="w-10 h-6 bg-slate-300 rounded-full shadow-inner transition-colors duration-200" :class="active ? 'bg-indigo-500' : ''"></div>
                                <div class="dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full shadow transition-transform duration-200 select-none"></div>
                            </div>
            ...
            <div class="space-y-2">
                ...
                <div class="relative group">
                    ...
                    <textarea name="location" id="location" rows="3"
                        class="block w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200"
                        placeholder="Complete address of the branch...">{{ old('location', $branch->location) }}</textarea>
                </div>
