...
@push('scripts')
    <script>
        ...
        function showDispenseModal(prescriptionId) {
            ...
            document.getElementById('quantityDispensed').max = selected.medicine.stock;
            
            // Auto populate batch number
            const batchInput = document.getElementById('batchNumberInput');
            if (selected.medicine.batches && selected.medicine.batches.length > 0) {
               const activeBatch = selected.medicine.batches.find(b => b.remaining_quantity > 0 && new Date(b.expiry_date) > new Date()) || selected.medicine.batches[0];
               batchInput.value = activeBatch.batch_number;
            } else {
               batchInput.value = '';
            }

            // Update form action
            document.getElementById('dispenseForm').action = `/pharmacy/prescriptions/${prescriptionId}/dispense`;
