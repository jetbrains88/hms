...
@section('breadcrumb', 'Pharmacy / Medicines / Edit')

@section('content')
<div class="max-w-4xl mx-auto animate-fade-in">
    <div class="flex items-center gap-4 mb-8">
        <a href="{{ route('pharmacy.medicines.show', $medicine) }}" class="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-slate-400 hover:text-blue-600 hover:border-blue-100 transition-all shadow-sm">
            <i class="fas fa-arrow-left"></i>
        </a>
        <div>
            <h2 class="text-3xl font-black text-slate-800 leading-tight">Edit Medicine</h2>
            <p class="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-1">Updates specifications for {{ $medicine->name }}</p>
        </div>
    </div>

    <form action="{{ route('pharmacy.medicines.update', $medicine) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-[2rem] p-8 shadow-xl shadow-slate-200/50 border border-slate-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="space-y-2">
                    ...
                    <select name="form_id" required class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-blue-500 transition-all font-bold text-slate-700 appearance-none">
                        ...
                        @foreach($forms as $form)
                            <option value="{{ $form->id }}" {{ old('form_id', $medicine->form_id) == $form->id ? 'selected' : '' }}>{{ $form->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="strength_value" value="{{ old('strength_value', $medicine->strength_value) }}" 
                        class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-blue-500 transition-all font-bold text-slate-700">
                </div>

                <div class="space-y-2">
                    <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Strength Unit</label>
                    <input type="text" name="strength_unit" value="{{ old('strength_unit', $medicine->strength_unit) }}" 
                        class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-blue-500 transition-all font-bold text-slate-700">
                </div>
                ...
                <div class="flex items-center gap-4 pt-6">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="requires_prescription" value="1" {{ old('requires_prescription', $medicine->requires_prescription) ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="mt-8 space-y-2">
                ...
                <textarea name="description" rows="3" 
                    class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-blue-500 transition-all font-medium text-slate-700">{{ old('description', $medicine->description) }}</textarea>
            </div>
        </div>

        <div class="flex items-center justify-end gap-4 pb-10">
            <a href="{{ route('pharmacy.medicines.show', $medicine) }}" class="px-8 py-4 bg-slate-100 text-slate-500 rounded-2xl font-bold hover:bg-slate-200 transition-all">Cancel</a>
